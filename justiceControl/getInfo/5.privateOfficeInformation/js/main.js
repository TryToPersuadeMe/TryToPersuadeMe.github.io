...
class SelectReport {
  ...
  buttonClick() {
    document.addEventListener("click", () => {
      if (
        !event.target.offsetParent.classList.contains("boxInformationWithCheckbox_valid") &&
        event.target.offsetParent.classList.contains("boxInformationWithCheckbox")
      ) {
  ...
  changeState(currentEl) {
    currentEl.classList.toggle("boxInformationWithCheckbox_show");
    setTimeout(() => {
      currentEl.classList.toggle("boxInformationWithCheckbox_animation");
    }, 100);
  }
}

const selectReport = new SelectReport({
  button: ".boxInformationWithCheckbox__selectButton",
  content: ".boxInformationWithCheckbox__container-js",
  arrow: ".boxInformationWithCheckbox__arrow",

  item: ".boxInformationWithCheckbox",
});
;
class ChangeDataform {
  constructor(props) {
    this.triggerButton = document.querySelectorAll(props.triggerButton);
    this.form = document.querySelectorAll(props.form);
    this.saveButton = document.querySelectorAll(props.saveButton);
    this.click();
  }
  ...
  click() {
    ...
    for (let index = 0; index < this.triggerButton.length; index++) {
      this.triggerButton[index].addEventListener("click", () => {
        this.form[index].classList.add("active");
      });

      this.saveButton[index].addEventListener("click", () => {
        this.form[index].classList.remove("active");
      });
    }
  }
}
...
const changeDataform = new ChangeDataform({
  ...
  form: ".changeData",
  saveButton: ".changeData__checkmark",
});
;
